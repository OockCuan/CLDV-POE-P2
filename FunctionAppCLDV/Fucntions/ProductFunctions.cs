            ...
            string id)
        {
            ...
            try
            {
                ...
                if (contentType.StartsWith("multipart/form-data", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (form.Text.TryGetValue("Price", out var priceTxt) && double.TryParse(priceTxt, out var price)) e.Price = (decimal)price;
                    if (form.Text.TryGetValue("StockAvailable", out var stockTxt) && int.TryParse(stockTxt, out var stock)) e.StockAvailable = stock;
                    if (form.Text.TryGetValue("ImageUrl", out var lu)) e.ImageUrl = lu;
                ...
                else
                {
                    ...
                    if (body.TryGetValue("Price", out var pr) && double.TryParse(pr?.ToString(), out var price)) e.Price = (decimal)price;
